...
void RespondToDetection(float close_score, float open_score) {
  int close_score_int = (close_score) * 100 + 0.5;
  int open_score_int = (open_score) * 100 + 0.5;
#if DISPLAY_SUPPORT
    if (!camera_canvas) {
      create_gui();
    }
    ...
    bsp_display_unlock();
#endif // DISPLAY_SUPPORT
  MicroPrintf("close score: %d%%, open score: %d%%",
              close_score_int, open_score_int);
}
